...
/**
 * Utility logic for working with Gameplay Abilities and related logic.
 */
namespace TemlCalculationUtils
{
	float CalculateProficiencyBonus(const FGameplayTag TagPrefix, const FGameplayEffectSpec& Spec)
	{
		return CalculateProficiencyBonus(TagPrefix.GetTagName(), Spec);
	}

	float CalculateProficiencyBonus(const FName TagPrefix, const FGameplayEffectSpec& Spec)
	{
		return CalculateProficiencyBonus(TagPrefix.GetPlainNameString(), Spec);
	}

	float CalculateProficiencyBonus(const FString TagPrefix, const FGameplayEffectSpec& Spec)
	{
